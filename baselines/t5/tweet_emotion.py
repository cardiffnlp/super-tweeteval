...
def train(model_name: str, model_low_cpu_mem_usage: bool, dataset: str, dataset_name: str, dataset_column_label: str,
    ...
    for s, (s_dataset, down_sample) in dataset_split.items():
        ...
        label_list = tmp.features[dataset_column_label].feature.names
        tmp.shuffle(random_seed)
        for i in tmp:
            model_inputs = tokenizer(i[dataset_column_text], truncation=True)
            model_inputs['labels'] = tokenizer(
                text_target=",".join([_l for _i, _l in zip(i[dataset_column_label], label_list) if _i == 1]), truncation=True)['input_ids']
    ...
    if dataset_split_test is not None and not os.path.exists(f"{output_dir}/model/evaluation_metrics.json"):
        ...
        label_list = tmp.features[dataset_column_label].feature.names
        _references = [set([_l for __i, _l in zip(_i[dataset_column_label], label_list) if __i == 1]) for _i in tmp]
        _scores = []
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Seq2Seq LM Fine-tuning on emotion classification.')
    ...
    parser.add_argument('--dataset-name', default="tweet_emotion", type=str)
