...
def train(model_name: str, model_low_cpu_mem_usage: bool, dataset: str, dataset_name: str, dataset_column_label: str,
    ...
    if dataset_split_test is not None and not os.path.exists(f"{output_dir}/model/evaluation_metrics.json"):
        ...
        if not os.path.exists(f"{output_dir}/model/prediction_test.txt"):
            ...
            input_data = [f"score intimacy: {i[dataset_column_text]}" for i in dataset_instance[dataset_split_test]]
    ...
    if model_alias is not None:
        ...
        sample = [f"score intimacy: {i[dataset_column_text]}" for i in dataset_instance[dataset_split_train]]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Seq2Seq LM Fine-tuning on tweet intimacy.')
    ...
    parser.add_argument('--dataset-name', default="tweet_intimacy", type=str)
